...
| v24.0 | 2026-09-17 | **Hito 28 (deploy directo para Rafael, túnel SSH restringido, cotización membretada):** (1) El Arquitecto aclaró que `pittsburgh.tourfindy.com` es entorno de pruebas del servidor, no el dominio final de producción — con eso, `deploy.yml` cambió para que `collab/rafa` despliegue a `./` (raíz pública) en vez de `./preview-rafa/`: cada push de Rafael ahora se refleja de inmediato en el sitio, sin pasar por PR. El Ruleset de GitHub sobre la rama `main` no se tocó — es exclusivamente un cambio de destino FTP; contrapartida documentada: cualquier push futuro a `main` debe traer primero lo último de `collab/rafa` o lo pisará (misma raíz remota). (2) **Acceso a base de datos para Rafael:** se investigó dar acceso remoto directo (MySQL sin túnel) — confirmado que, aunque el ACL de cPanel para Remote MySQL ya tiene `%` (cualquier IP) autorizado desde antes, una prueba real de conexión directa a `chir205.websitehostserver.net:3306` se queda sin responder (mismo firewall de red diagnosticado en el Hito 12/16, sigue vigente). Se generó una llave SSH ed25519 nueva y exclusiva para Rafael, agregada a `authorized_keys` del servidor con `command="/bin/false"` + `permitopen="localhost:3306"` — se probaron los 3 escenarios antes de entregarla: (a) intento de ejecutar comandos/listar `/home` → bloqueado; (b) túnel hacia MySQL → exitoso, datos reales confirmados (10 banners); (c) intento de reenviar hacia otro puerto (22) → rechazado explícitamente por el servidor ("administratively prohibited"). Entregable: llave privada + script `tunnel_bd_rafa.sh` + instructivo, para que se conecte a `tourfindycom_pittsburgh_preview` (su base aislada, nunca la real) desde su propia máquina. (3) `Propuesta/` se convirtió en hoja membretada real: datos de ACADEP (dirección, web, celular) provistos por el Arquitecto, número de cotización formal (`COT-ACADEP-2026-001`), y los 2 costos re-etiquetados como servicios independientes (no "Propuesta 1/2" alternativas) por corrección explícita del Arquitecto. |
