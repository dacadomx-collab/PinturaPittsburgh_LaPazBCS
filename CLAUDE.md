...
| Diagnóstico visual de conexión (Hito 15) | ✅ `test_db.php` (raíz del proyecto, protegido por `SETUP_TOKEN`, se autoniega en producción — reemplaza a `test_conexion.php` del Hito 14). `GET ?token=...` → prueba PDO en orden `DB_HOST` configurado → `localhost` → `127.0.0.1`, se detiene en el primer éxito y muestra un panel verde (host ganador, ms de respuesta, tablas, usuarios — con checklist de los 2 emails esperados) o un panel rojo con la categoría de error (`red`/`credenciales`/`bd_inexistente`/`desconocido`) por cada host intentado. Pensado para ejecutarse DIRECTAMENTE en el servidor de staging — confirmado localmente (Hito 15) que contra el MySQL de XAMPP, `localhost`/`127.0.0.1` sí responden pero con `Access Denied` (credenciales de staging que no existen ahí), validando que el diagnóstico distingue correctamente red vs. credenciales. **URL con el token ya resuelto:** ver informe de cierre de este Hito — nunca se gestiona el token a mano. |
