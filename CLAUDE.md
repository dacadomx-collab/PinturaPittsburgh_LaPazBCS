...
**Versión:** 10.0 | **Fecha:** 2026-09-12 | **Arquitecto:** [NOMBRE_ARQUITECTO — pendiente de confirmar]

**Estado del proyecto:** Sitio público (landing, catálogo, PDP, checkout) y panel administrativo completo — dashboard PHP unificado (`admin/index.php` + `admin/layout/*.php`: login con redirección por rol, catálogo/precios, pedidos, publicador social, asistente IA) — implementados y verificados estructuralmente (`php -l`, `node --check`, pruebas HTTP en vivo contra XAMPP local). Endpoints públicos de banners/promociones/feed de publicaciones (Contratos 10/11/12) implementados y probados, pero **`index.html` todavía no incluye su marcado real** — el "Data Contract Frontend" que debe seguir ya está documentado (`knowledge/07` §3) junto con `mock-data/mock-data.json`, y construir esas 3 secciones es trabajo de frontend dentro del alcance del colaborador externo (§13), corrección hecha en el Hito 13 tras detectar que el estatus previo lo daba por completo indebidamente. Colaboración externa Zero-Trust activa (rol `colaborador`, guarda de sesión real, suite de pruebas en `scripts/test_flujo_colaborador.{sh,bat}`). Toggle Día/Noche y botón "Volver Arriba" en todas las pantallas públicas/admin. `modulos/` adoptado formalmente como hoja de ruta de características (§14). Entorno de staging asignado (§6): `.env` local ya conecta hasta la capa PDO real; los Secrets de FTP SÍ están configurados y el deploy a staging funciona (corrección Hito 13, ver §8). **Bloqueantes activos (Hito 14):** (1) los 2 usuarios (`dacadomx@yahoo.com` admin y `armandocastillejos086@gmail.com` colaborador) **ya fueron insertados exitosamente en la BD remota** por el Arquitecto — el bloqueo restante es puramente de conectividad de red: el puerto 3306 sigue sin responder desde este entorno de desarrollo, y `test_conexion.php` (nuevo, §6/§7) documenta el hallazgo de que el `.env` DEL SERVIDOR de staging probablemente necesita `DB_HOST=localhost` en vez del hostname público — pendiente de confirmar corriendo ese script directamente en el servidor; (2) `https://pittsburgh.tourfindy.com/` sigue devolviendo 403 en toda ruta, incluida una inexistente — se auditó y reforzó `.htaccess` (§6), pero el rastreo lógico de sus reglas confirma que la causa real está fuera de este archivo (permisos de archivo o Document Root en cPanel); (3) falta la contraseña real de SMTP (nunca recibida ni inventada).
...
├── test_conexion.php                ← ⚠️ Temporal (Hito 14) — diagnóstico categorizado de conexión BD, protegido por SETUP_TOKEN
│
...
| Diagnóstico de conexión (Hito 14) | ✅ `test_conexion.php` (raíz del proyecto, protegido por `SETUP_TOKEN`, se autoniega en producción — mismo criterio que la fila anterior). `GET ?token=...` → intenta PDO directo (no vía `Database::getConnection()`, que atrapa su propia excepción — ver comentario del archivo), categoriza el fallo en `red`/`credenciales`/`bd_inexistente`/`consulta`/`desconocido` con una guía técnica en pantalla, y si conecta, corre `SELECT id, email, role, estatus FROM users` a través de `api/conexion.php` para confirmar que los 2 usuarios ya insertados son visibles. Pensado para ejecutarse DIRECTAMENTE en el servidor de staging (no solo desde este entorno local) — es la forma más directa de confirmar si el hallazgo de `DB_HOST=localhost` de la fila anterior resuelve la conectividad. |
| 403 "Acceso denegado" en `https://pittsburgh.tourfindy.com/` | ⚠️ **Diagnóstico 2026-09-12 (Hito 12), hardening 2026-09-12 (Hito 14):** el `.htaccess` de este repo SÍ llegó al servidor y está activo (el cuerpo de la respuesta 403 es exactamente el `ErrorDocument 403 "Acceso denegado."` de este mismo `.htaccess`). Prueba decisiva: **hasta una ruta aleatoria e inexistente** (`esto-no-existe-de-verdad-12345.html`) devuelve 403 en vez de 404. Se rastreó la lógica completa de NIVEL 3/4/5 (bloqueo por carpeta → pase directo de archivos/carpetas existentes → front controller comentado) para una ruta que NO existe: ninguna `RewriteRule` de carpeta coincide, la condición `-f`/`-d` del NIVEL 4 falla (el archivo no existe), y el NIVEL 5 está comentado — **conclusión: bajo esta lógica, una ruta inexistente debería dar 404, no 403, con o sin la corrección de esta fila.** Por autorización explícita del Hito 14 se agregó de cualquier forma, como defensa en profundidad (nunca de más, nunca puede ser la causa del problema): `Require all granted` explícito en NIVEL 0 y `DirectoryIndex index.php index.html` (orden invertido, PHP primero) — verificado que no rompe nada en local (`knowledge/`, `.env` y demás carpetas sensibles del NIVEL 1/3 se mantienen bloqueadas). **Esto NO reemplaza el diagnóstico original:** el problema sigue apuntando, con evidencia más fuerte que nunca, a **permisos de archivo/carpeta** (directorios FTP en `750`/`700` en vez de `755`) o a un **Document Root mal configurado** en cPanel para el subdominio — algo que ocurre en una capa que el propio `.htaccess` no puede controlar ni diagnosticar. **Acción pendiente del Arquitecto:** en cPanel → Administrador de Archivos (o SSH, ver `modulos/MODULO_03_CONEXION_SSH_HOSTING.md` §Fase 4 "Matriz de Reconocimiento", adoptado en §14), verificar permisos `755`/`644` y el Document Root exacto del subdominio. |
...
| `test_conexion.php` (raíz, HTML no JSON) | GET | `SETUP_TOKEN` (nunca sesión de usuario) | — | ⚠️ Temporal — ver §6. Se niega en `APP_ENV=production`. Eliminar tras usarlo. |
...
**Estado:** ✅ Activo y funcional — los Secrets de FTP ya están dados de alta y el pipeline entrega correctamente a staging (`pittsburgh.tourfindy.com`). Corrección 2026-09-12 (Hito 13): el estatus anterior ("Secrets pendientes") ya no era exacto — el Hito 12 confirmó indirectamente que el deploy SÍ llega al servidor (el `.htaccess` de este repo es el que responde en vivo, ver §6). El 403 diagnosticado ahí es un problema de permisos/Document Root en el hosting, no del pipeline de CI/CD.
